...
import { printReviews } from "../../reducers/Slice/reviewSlice";
...
const AboutGame = () => {
  ...
  const promos = useAppSelector((state) => state.promoReducer.promo);

  const game = useAppSelector((state) => state.gameReducer.game).find(
    (item) => item._id === gameId
  );
  console.log(game);
  const favourites = useAppSelector(
    (state) => state.favoriteReducer.favourites
  );
  const tokenId = useAppSelector((state) => state.registrationReducer.id);

  const isExistenceReview = useAppSelector(
    (state) => state.reviewSlice.isExistenceReview
  );

  const userID = useAppSelector(
    (state) => state.registrationReducer.userID?.id
  );
      ...
      <div className={style.card_information}>
        ...
        <div className={style.information_game}>
          ...
          <button
            ...
            className={style.add_toFavorites}
          >
